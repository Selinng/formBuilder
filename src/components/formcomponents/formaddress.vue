      	...
      	<div class="form-group">
      		<div class="col-sm-4">
	            <select class="form-control" v-model="province">
	                ...
	                <option v-for="item in getProvince" :key="item.item_code" :value="item.item_code">{{item.item_name}}</option>
	            </select>
            </div>
            <div class="col-sm-4">
	            <select class="form-control" v-model="city">
	                <option value="0">请选择城市</option>
	                <option v-for="item in getCity" :key="item.item_code" :value="item.item_code">{{item.item_name}}</option>
	            </select> 
             </div>
            <div class="col-sm-4">
	            <select class="form-control" v-model="county">
	                <option value="0">请选择区县</option>
	                <option v-for="item in getCounty" :key="item.item_code" :value="item.item_code">{{item.item_name}}</option>
	            </select>
            </div>
        </div>
...
const PROVINCE_LIST = ADDRESS_JSON.filter(item => item.item_code.substr(2, 4) === '0000')
const CITY_LIST = ADDRESS_JSON.filter(item => item.item_code.substr(2, 2) !== '00' && item.item_code.substr(4, 2) === '00')
const COUNTY_LIST = ADDRESS_JSON.filter(item => item.item_code.substr(4, 2) !== '00')
export default {
	data () {
		return {
			province: '0',
	...
	computed: {
		...
		getCity () {
			this.city = '0'
			this.county = '0'
			return CITY_LIST.filter(item => this.province.substr(0, 2) === item.item_code.substr(0, 2))
		},

		getCounty () {
			this.county = '0'
			return COUNTY_LIST.filter(item => this.city.substr(0, 4) === item.item_code.substr(0, 4))
		}
	},

	methods: {
		// areaMenuResponse () {
		// 	this.province = CITY_JSON.filter(item => item.item_code.substr(2, 4) === '0000')
		// }
	}
}
</script>
