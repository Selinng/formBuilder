...
const PROVINCE_LIST = ADDRESS_JSON.filter(
  item => item.item_code.substr(2, 4) === "0000"
);
const CITY_LIST = ADDRESS_JSON.filter(
  item =>
    item.item_code.substr(2, 2) !== "00" && item.item_code.substr(4, 2) === "00"
);
const COUNTY_LIST = ADDRESS_JSON.filter(
  item => item.item_code.substr(4, 2) !== "00"
);
export default {
  data() {
    return {
  ...
  computed: {
    ...
    getCity() {
      ...
      return CITY_LIST.filter(
        item => this.province.substr(0, 2) === item.item_code.substr(0, 2)
      );
    },
